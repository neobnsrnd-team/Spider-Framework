...
### 결재 워크플로우
- 생성 코드 승인/반려 처리
- 승인 시 `.tsx` 파일을 지정 경로에 저장 (`REACT_APPROVAL_OUTPUT_DIR`)
...
Claude API 호출 시 전달하는 system prompt는 `reactive-springware` 패키지에서 추출한 md 파일 4개를 조합하여 만든다.
...
| `extract-component-map.ts` | `docs/component-map.md` | `generated/component-map.md` | Figma → React 매핑 전략 문서를 그대로 복사 |

`CLAUDE.md`는 `docs/CLAUDE.md`를 수동으로 관리하며, 스크립트 대상이 아니다.

### 파일 배치 (reactPlatform)

생성된 md 파일과 `CLAUDE.md`를 `src/main/resources/prompts/`에 배치한다.

```
src/main/resources/prompts/
├── CLAUDE.md           ← docs/CLAUDE.md (수동 관리)
├── component-types.md  ← generated/component-types.md
├── design-tokens.md    ← generated/design-tokens.md
└── component-map.md    ← generated/component-map.md
```

### 로딩 및 조합 (PromptLoader / PromptBuilder)

`PromptLoader`는 애플리케이션 시작 시(`@PostConstruct`) 4개의 파일을 읽어 메모리에 캐싱한다.
...
| `REACT_APPROVAL_OUTPUT_DIR` | — | `./generated` | 승인된 .tsx 파일 저장 경로 |
