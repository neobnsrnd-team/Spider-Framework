...
**작업 내용**
- [x] `#sqModalSqlType` `<input type="text">` → `<select>` 로 교체  
  - 옵션: `R (SELECT)` / `C (INSERT)` / `U (UPDATE)` / `D (DELETE)` / `P (PROCEDURE)` / `I (SQL)`
- [x] `#sqModalExecType` `<input type="text">` → `<select>` 로 교체  
  - 옵션: `O (ONLINE)` / `B (BATCH)`
- [x] `_fillForm()` 에서 `.val()` 로 select 값이 올바르게 채워지는지 확인
- [x] `_setFieldsReadonly()` 에서 select 에 `.prop('disabled')` 적용 확인

**테스트 항목**
- [x] 목록에서 기존 쿼리 행 클릭 → 모달 오픈 시 `sqlType`, `execType` 드롭다운에 DB 저장값이 자동 선택되는가
- [x] 신규 등록 모달 오픈 시 드롭다운이 빈 상태(선택 없음)로 초기화되는가
- [x] 읽기 권한만 있는 계정으로 접속 시 드롭다운이 비활성화(disabled)되는가
- [x] 수정 후 저장 → 목록 새로고침 → 다시 클릭했을 때 변경된 값이 유지되는가
...
**작업 내용**
- [x] `#sqModalSqlQuery` textarea 레이블 아래, textarea 위에 툴바 div 추가
  ```
  [페이징 쿼리 추가] [프로시저 호출] [쿼리 검색] [확대]
  ```
- [x] `SqlQueryModal.addPagingQuery()` 함수 구현  
  - textarea 현재 내용 + Oracle ROWNUM 기반 페이징 템플릿 (`#startRow#`, `#endRow#`) 삽입
- [x] `SqlQueryModal.callProcedure()` 함수 구현  
  - CALL 문법 + IN/OUT 파라미터 주석 포함 템플릿 삽입
- [x] 버튼 클릭 이벤트 바인딩 (`#btnAddPaging`, `#btnCallProcedure`)
- [x] 읽기 전용 모드(상세 조회, 권한 없음)일 때 툴바 버튼 비활성화 처리

**테스트 항목**
- [x] 쿼리 등록 모달에서 `[페이징 쿼리 추가]` 클릭 → textarea에 Oracle 페이징 템플릿이 삽입되는가
- [x] 이미 내용이 있는 textarea에 삽입 시 기존 내용이 유지되는가 (append)
- [x] `[프로시저 호출]` 클릭 → CALL 문법 템플릿이 삽입되는가
- [x] OUT 파라미터 형식(`{out, jdbcType=VARCHAR, ...}`)이 정확히 삽입되는가
- [x] 읽기 전용 모드에서 툴바 버튼이 비활성화되어 있는가
...
**작업 내용**
- [x] `SqlQueryModal._validateQuery()` 함수 구현
...
- [x] `save()` 내 `_validateForm()` 호출 다음에 `_validateQuery()` 순차 호출
- [x] 검증 실패 시 `Toast.warning()` 으로 구체적인 오류 메시지 표시

**테스트 항목**
- [x] `sqlType = R` 인데 쿼리가 `UPDATE ...` 로 시작 → 저장 차단 및 경고 메시지 표시되는가
- [x] `<ISNULL>` 태그가 포함된 쿼리 → 저장 차단 및 경고 메시지 표시되는가
- [x] `sqlType = P` 이고 `CALL 프로시저명(...)` 쿼리 → 정상 저장되는가
- [x] `sqlType = I` (SQL) 이고 `ALTER TABLE ...` 쿼리 → 정상 저장되는가
- [x] `sqlType` 미선택(빈 값) 상태에서 저장 → 키워드 검증 없이 필수 필드 검증(`_validateForm`)만 동작하는가
...
**작업 내용**
- [x] 백엔드 팀에 테스트 실행 API 엔드포인트 및 스펙 확인
...
- [x] 모달 Footer에 `[쿼리 테스트]` 버튼 추가 (상세 모드 + 쓰기 권한 시에만 표시)
- [x] `SqlQueryModal.testQuery()` 함수 구현
  - 확인된 API 엔드포인트로 `window.open()` 팝업 실행
  - 팝업 사이즈: `width=1200, height=800, scrollbars=yes, resizable=yes`
- [x] 버튼 클릭 이벤트 바인딩 (`#btnSqlQueryTest`)

**테스트 항목**
- [x] 기존 쿼리 상세 모달에서 `[쿼리 테스트]` 버튼이 보이는가 (쓰기 권한 계정)
- [x] 읽기 권한 계정에서는 버튼이 숨겨지는가
- [x] 신규 등록 모달(create 모드)에서는 버튼이 숨겨지는가
- [x] 버튼 클릭 시 적절한 파라미터(queryId, sqlGroupId, dbId)가 팝업 URL에 포함되는가
- [x] 테스트 팝업이 정상적으로 열리는가
...
**작업 내용**
- [x] `SqlQuerySearchRequest` — `sqlGroupId`, `sqlGroupName`, `sqlType` 필드 추가
- [x] `SqlQueryMapper` — 3개 검색 메서드에 신규 파라미터 추가
- [x] `SqlQueryMapper.xml` — `searchConditions` 프래그먼트에 3개 조건 추가, `countAllWithSearch`에 LEFT JOIN 추가
- [x] `SqlQueryService` — 신규 파라미터 전달
- [x] `sqlquery-manage.html` — `SQL 그룹 ID`, `SQL 그룹 명`, `SQL 유형` 검색 필드 3개 추가

**테스트 항목**
- [x] `SQL GROUP ID` 입력 후 검색 → 해당 그룹의 쿼리만 필터링되는가
- [x] `SQL GROUP 명` 부분 검색(LIKE) → 결과가 올바른가
- [x] `SQL 유형` 드롭다운에서 `R (SELECT)` 선택 → SELECT 쿼리만 표시되는가
- [x] 여러 조건 동시 입력 후 검색 → AND 조건으로 복합 필터링되는가
- [x] 검색 조건 초기화 버튼 클릭 → 추가된 3개 필드도 함께 초기화되는가
...
**작업 내용**
- [x] 툴바에 `[쿼리 검색]` 버튼 추가, 클릭 시 검색 바(`#sqSearchBar`) 토글 표시
- [x] `SqlQueryModal.searchQuery(direction)` 구현 — `indexOf` 전체 매칭 수집 → `setSelectionRange` 선택
- [x] `[◀]` / `[▶]` 이전·다음 버튼으로 매칭 순환, `N / M` 카운트 표시
- [x] 검색어 없음 또는 미발견 시 `Toast.warning()` 표시
- [x] `Esc` 키 및 `[✕]` 버튼으로 검색창 닫기, 모달 닫힐 때 자동 초기화
- [x] 읽기 전용 상태에서도 `[쿼리 검색]` 버튼은 활성 유지

**테스트 항목**
- [x] `[쿼리 검색]` 버튼 클릭 → 검색 입력창이 표시되는가
- [x] 검색어 입력 후 Enter 또는 `[▶ 다음]` → textarea 내 첫 번째 매칭 구간이 선택(파란 하이라이트)되는가
- [x] `[▶ 다음]` 반복 클릭 → 다음 매칭 위치로 이동하는가
- [x] `[◀ 이전]` 클릭 → 이전 매칭 위치로 이동하는가
- [x] 마지막 매칭에서 `[▶ 다음]` → 첫 번째 매칭으로 순환하는가
- [x] 존재하지 않는 검색어 입력 → 경고 메시지 표시되는가
- [x] `[✕]` 또는 `Esc` → 검색창이 닫히는가
...
**목표**  
긴 쿼리 작성 시 편의를 위해 SQL Query textarea를 확대/축소할 수 있는 토글 기능을 구현한다.

**변경 파일**
- `admin/src/main/resources/templates/pages/sqlquery-manage/sqlquery-modal.html`

**작업 내용**
- [x] 툴바에 `[확대]` 버튼 추가 (구버전 확인: textarea height 확대 방식, 모달 fullscreen 아님)
- [x] `toggleFullscreen()` — textarea height를 500px로 확대 / 원래 크기로 복귀 토글
- [x] 버튼 아이콘 `bi-arrows-fullscreen` ↔ `bi-fullscreen-exit` 교체
- [x] 모달 닫힐 때 자동 축소 (`_exitFullscreen`)

**테스트 항목**
- [x] `[확대]` 버튼 클릭 → textarea가 500px로 확대되는가 (모달 크기 유지)
- [x] 확대 후 버튼이 `[축소]`로 바뀌는가
- [x] `[축소]` 클릭 → textarea가 기본 크기로 복귀하는가
- [x] 확대 상태에서 저장/삭제/닫기 버튼이 정상 동작하는가
- [x] 모달 닫은 후 재오픈 시 기본 크기로 시작하는가
- [x] 읽기 전용 상태에서도 `[확대]` 버튼이 동작하는가
...
- `admin/src/main/java/com/example/admin_demo/domain/sqlquery/dto/SqlQueryHistoryResponse.java`

**변경 파일**
- `admin/src/main/resources/templates/pages/sqlquery-manage/sqlquery-modal.html` (이력/복원 버튼 추가)
- `admin/src/main/resources/templates/pages/sqlquery-manage/sqlquery-manage.html` (신규 모달 include)
- `admin/src/main/java/com/example/admin_demo/domain/sqlquery/mapper/SqlQueryMapper.java`
- `admin/src/main/resources/mapper/oracle/sqlquery/SqlQueryMapper.xml`
- `admin/src/main/java/com/example/admin_demo/domain/sqlquery/service/SqlQueryService.java`
- `admin/src/main/java/com/example/admin_demo/domain/sqlquery/controller/SqlQueryController.java`

**작업 내용**
- [x] 백엔드: `SqlQueryHistoryResponse` DTO 신규 생성
- [x] 백엔드: `GET /api/sql-queries/{queryId}/history` — 이력 목록 (최신순)
- [x] 백엔드: `GET /api/sql-queries/{queryId}/history/{versionId}` — 이력 단건 조회
- [x] 백엔드: `POST /api/sql-queries/{queryId}/restore/{versionId}` — 복원 (복원 전 현재 상태 자동 백업)
- [x] 복원 모달 HTML 구조 작성 (상단 이력 목록 테이블 + 하단 SQL 좌우 비교 패널)
- [x] `SqlQueryRestoreModal` JS 객체 구현
  - `open(queryId, currentSqlQuery)` — 모달 오픈, 이력 목록 로드
  - `_selectVersion(versionId)` — 행 선택 시 이력 SQL 우측 패널에 표시
  - `restore()` — 확인 대화 후 복원 API 호출 → 성공 시 두 모달 닫기 + 목록 새로고침
- [x] `sqlquery-modal.html` Footer에 `[이력/복원]` 버튼 추가 (상세 모드 + W 권한)
- [x] `sqlquery-manage.html` 에 복원 모달 `th:replace` include 추가

**테스트 항목**
- [x] 상세 모달 footer에 `[이력/복원]` 버튼이 표시되는가 (쓰기 권한 필요)
- [x] `[이력/복원]` 클릭 → 복원 모달이 열리는가
- [x] 이력이 없는 쿼리 → "저장된 이력이 없습니다" 메시지가 표시되는가
- [x] 이력이 있는 쿼리 → 백업일시/사유/수정자가 목록에 표시되는가
- [x] 이력 행 클릭 → 하단 SQL 비교 영역이 열리고 좌(현재)/우(이력) SQL이 표시되는가
- [x] `[복원]` 버튼 클릭 → 확인 다이얼로그 → 복원 후 두 모달 닫히고 목록 새로고침되는가
- [x] 복원 후 `[이력/복원]` 재진입 시 복원 직전 상태가 이력에 자동 추가되어 있는가
...
**작업 내용**
- [x] 백엔드: `updateUseYn()` mapper 메서드 + `UPDATE SET USE_YN` SQL 추가
- [x] 백엔드: `SqlQueryService.toggleUseYn()` — 현재값 조회 후 Y↔N 반전
- [x] 백엔드: `PATCH /api/sql-queries/{queryId}/use-yn` 엔드포인트 추가 (W 권한)
- [x] `PAGE_CONFIG.table.columns` 의 `useYn` 렌더러에 쓰기 권한 시 클릭 가능 배지 추가 (`data-query-id`, `data-use-yn` 속성)
- [x] `SqlQueryPage.toggleUseYn(queryId, currentVal)` 구현 — `Toast.confirm()` → `PATCH` 호출
- [x] `mousedown` 플래그(`_preventRowClick`)로 배지 클릭 시 상세 모달 동시 열림 차단

**테스트 항목**
- [x] 목록에서 `useYn = Y` 배지 클릭 → 확인 다이얼로그 표시되는가
- [x] 확인 클릭 → `useYn = N` 으로 변경되고 목록이 새로고침되는가
- [x] `useYn = N` 배지 클릭 → `useYn = Y` 로 변경되는가
- [x] 배지 클릭 시 상세 모달이 함께 열리지 않는가
- [x] 읽기 전용 계정에서 배지가 클릭되지 않는가
- [x] 취소 클릭 → 변경 없이 현재 값 유지되는가
...
**작업 내용**
- [x] `PAGE_CONFIG.table.columns` 에 `timeOut` 컬럼 추가 (`width: '80px'`, `sortable: false`)
- [x] 백엔드 `findAllWithSearch` SQL에 `TRIM(q.TIME_OUT) AS timeOut` 이미 포함 확인

**테스트 항목**
- [x] 목록 테이블에 `Timeout` 컬럼이 표시되는가
- [x] 타임아웃 설정값이 있는 쿼리의 값이 올바르게 표시되는가
- [x] 값이 없는 경우 빈 값으로 표시되는가
...
**변경 파일**
- `admin/src/main/resources/templates/pages/sqlquery-manage/sqlquery-modal.html`
- `admin/src/main/java/com/example/admin_demo/domain/sqlquery/dto/SqlGroupResponse.java` (신규)
- `admin/src/main/java/com/example/admin_demo/domain/sqlquery/mapper/SqlQueryMapper.java`
- `admin/src/main/resources/mapper/oracle/sqlquery/SqlQueryMapper.xml`
- `admin/src/main/java/com/example/admin_demo/domain/sqlquery/service/SqlQueryService.java`
- `admin/src/main/java/com/example/admin_demo/domain/sqlquery/controller/SqlQueryController.java`
...
**작업 내용**
- [x] 백엔드: `SqlGroupResponse` DTO 신규 생성 (`groupId`, `groupName`)
- [x] 백엔드: `SqlQueryMapper.searchSqlGroups(keyword)` — `FWK_SQL_GROUP` 키워드 LIKE 검색, 최대 20건
- [x] 백엔드: `SqlQueryService.searchGroups(keyword)` 추가
- [x] 백엔드: `GET /api/sql-queries/group-search?keyword=xxx` 엔드포인트 추가 (R 권한)
- [x] `#sqModalSqlGroupId` 필드를 `position-relative` 래퍼로 감싸고 dropdown div 추가
- [x] 커스텀 autocomplete JS 구현 (300ms 디바운스, `groupId — groupName` 형식, 포커스 시 즉시 열기, 외부 클릭/모달 닫힘 시 정리)
- [x] 읽기 전용 모드에서 autocomplete 비활성화

**테스트 항목**
- [x] `SQL 그룹 ID` 필드에 타이핑 시 매칭 그룹 목록이 드롭다운으로 표시되는가
- [x] 목록에서 항목 선택 시 필드에 해당 ID가 채워지는가
- [x] 읽기 전용 모드에서 autocomplete가 비활성화되는가
